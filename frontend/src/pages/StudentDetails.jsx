      ...
      <div className="detail-container">
        ...
        <div className="sidebar-column">
          <div className="info-card">
            {!isEditing ? (
              <div>
                ...
                <button onClick={() => setIsEditing(true)} className="btn btn-primary" style={{ marginTop: '10px' }}>Edit Details</button>
              </div>
            ) : (
          ...
          <div className="info-card info-card-highlight">
            <h4 style={{ margin: '0 0 5px 0' }}>Revenue for Current Month:</h4>
            <p style={{ margin: 0, fontSize: '20px', fontWeight: 'bold', color: '#1cc88a' }}>${currentMonthRevenue}</p>
          </div>
        </div>
        ...
        <div className="main-content-column">
          ...
          <form onSubmit={handleSessionSubmit} className="form-grid" style={{ gridTemplateColumns: 'repeat(4, 1fr)' }}>
            <div>
            ...
            <button type="submit" className="btn btn-success" style={{ padding: '7px' }}>Submit Session</button>
          </form>
                ...
                <div key={s.id} className="session-history-card">
                  {!isCurrentSessionEditing ? (
                    <div>
                      ...
                      <div style={{ marginTop: '10px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          <button onClick={() => toggleNotes(s.id)} className="btn btn-secondary" style={{ padding: '4px 8px', fontSize: '12px', color: '#333', background: '#fff', border: '1px solid #ddd' }}>
                            {expandedNotes[s.id] ? 'Hide Notes' : 'View Notes'}
                          </button>
                          <button onClick={() => startEditingSession(s)} className="btn btn-primary" style={{ padding: '4px 8px', fontSize: '12px' }}>
                            Edit
                          </button>
                        </div>
                        <button onClick={() => handleDeleteSession(s.id)} className="btn btn-danger" style={{ fontSize: '12px' }}>Delete Record</button>
                      </div>
                      {expandedNotes[s.id] && (
                        <div style={{ marginTop: '12px', padding: '10px', backgroundColor: '#ffffff', borderRadius: '4px', fontSize: '14px', borderLeft: '4px solid #4e73df', boxShadow: 'inset 0 1px 3px rgba(0,0,0,0.05)' }}>
                    ...
                    <form onSubmit={(e) => handleSessionUpdateSubmit(e, s.id)} style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '10px' }}>
                      <div><label style={{ fontSize: '11px' }}>Date</label><input type="date" value={editSessionForm.date} onChange={e => setEditSessionForm({...editSessionForm, date: e.target.value})} required className="form-input" /></div>
                      <div>
                      ...
                      <div><label style={{ fontSize: '11px' }}>Mins</label><input type="number" step="15" value={editSessionForm.duration_minutes} onChange={e => setEditSessionForm({...editSessionForm, duration_minutes: e.target.value})} required className="form-input" /></div>
                      <div><label style={{ fontSize: '11px' }}>Rate ($)</label><input type="number" value={editSessionForm.hourly_rate} onChange={e => setEditSessionForm({...editSessionForm, hourly_rate: e.target.value})} required className="form-input" /></div>
                      <div style={{ gridColumn: 'span 3' }}>
                      ...
                      <div style={{ display: 'flex', gap: '5px', alignItems: 'end', justifyContent: 'flex-end' }}>
                        <button type="submit" className="btn btn-success" style={{ padding: '5px 10px', fontSize: '12px' }}>Save</button>
                        <button type="button" className="btn btn-secondary" onClick={() => setEditingSessionId(null)} style={{ padding: '5px 10px', fontSize: '12px' }}>Cancel</button>
                      </div>
                    </form>
                  )}
                </div>
...
//           // Force string comparison by splitting 'YYYY-MM-DD'
//           // This completely avoids the browser timezone shifting bug
//           const dateParts = session.date.split('-'); 
//           const sessionYear = parseInt(dateParts[0], 10);
//           const sessionMonth = parseInt(dateParts[1], 10) - 1; // Convert to 0-indexed month
